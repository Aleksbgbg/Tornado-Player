namespace Tornado.Player
{
    internal static class Constants
    {
        internal static readonly string[] SupportedMediaFormats =
        {
                "*.dat", ".3g2", ".3gp", ".3gp2", ".3gpp", ".aac", ".adt", ".adts", ".aif", ".aifc", ".aiff", ".amv", ".asf", ".avi", ".bin", ".cda", ".cue", ".divx", ".dv", ".flv", ".gxf", ".iso", ".m1v", ".m2t", ".m2ts", ".m2v", ".m4a", ".m4v", ".mkv", ".mov", ".mp2", ".mp2v", ".mp3", ".mp4", ".mp4v", ".mpa", ".mpe", ".mpeg", ".mpeg1", ".mpeg2", ".mpeg4", ".mpg", ".mpv2", ".mts", ".nsv", ".nuv", ".ogg", ".ogm", ".ogv", ".ogx", ".ps", ".rec", ".rm", ".rmvb", ".tod", ".ts", ".tts", ".vob", ".vro", ".wav", ".webm", ".wma", ".wmv"
        };

        internal static class DataStoreNames
        {
            internal const string Tracks = "Tracks";

            internal const string Playlists = "Playlists";

            internal const string ManagedPlaylists = "Managed Playlists";

            internal const string ActivePlaylist = "Active Playlist";

            internal const string PlaybarState = "Playbar State";

            internal const string AppLayout = "App Layout";

            internal const string Settings = "Settings";
        }
    }
}